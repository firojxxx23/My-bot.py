"""
VIP NUMBER BOT - Premium OTP Bot
============================================
Installation: pip install pyTelegramBotAPI requests flask
Run: python bot.py
============================================
"""
...
COUNTRY_FLAGS = {
    ...
    "zambia": "🇿🇲", "zimbabwe": "🇿🇼", "usa": "🇺🇸", "uk": "🇬🇧", "uae": "🇦🇪", "hong kong": "🇭🇰"
}

COUNTRY_ISO = {
    "bangladesh": "BD", "india": "IN", "pakistan": "PK", "indonesia": "ID",
    "united states": "US", "usa": "US", "united kingdom": "GB", "uk": "GB",
    "russia": "RU", "brazil": "BR", "nigeria": "NG", "philippines": "PH",
    "egypt": "EG", "turkey": "TR", "vietnam": "VN", "germany": "DE",
    "france": "FR", "italy": "IT", "spain": "ES", "japan": "JP",
    "china": "CN", "mexico": "MX", "canada": "CA", "australia": "AU",
    "saudi arabia": "SA", "uae": "AE", "malaysia": "MY", "singapore": "SG",
    "netherlands": "NL", "sweden": "SE", "poland": "PL", "ukraine": "UA",
    "south africa": "ZA", "kenya": "KE", "morocco": "MA", "ghana": "GH",
    "thailand": "TH", "south korea": "KR", "taiwan": "TW", "hong kong": "HK"
}

SERVICE_SHORTS = {
    "facebook": "FB", "whatsapp": "WA", "telegram": "TG", "instagram": "IG",
    "twitter": "TW", "x": "X", "google": "GO", "gmail": "GM", "youtube": "YT",
    "tiktok": "TT", "snapchat": "SC", "binance": "BN", "imo": "IMO",
    "messenger": "MS", "custom search": "CS"
}

EMOJI_COLLECTION = {
    "facebook": "📘", "whatsapp": "💚", "telegram": "✈️", "instagram": "📷",
    "twitter": "𝕏", "google": "🔍", "gmail": "📧", "youtube": "🎬",
    "tiktok": "🎵", "snapchat": "👻", "binance": "💰", "imo": "💭",
    "messenger": "💬", "custom search": "🔎",
    "done": "✅", "cross": "❌", "warning": "⚠️", "time": "⏰",
    "waiting": "🔄", "message": "📩", "otp": "🔐", "number": "📞",
    "world": "🌐", "user": "👤", "bot": "🤖", "live": "🟢", "off": "🔴",
    "traffic": "📊", "star": "⭐", "crown": "👑", "diamond": "💎",
    "fire": "🔥", "sparkles": "✨", "globe": "🌍", "pin": "📌",
    "note": "📝", "gear": "⚙️", "link": "🔗", "plus": "➕", "trash": "🗑️",
    "gift": "🎁", "shield": "🛡️", "key": "🔑", "lock": "🔒", "bell": "🔔",
    "rocket_launch": "🚀", "trophy": "🏆", "medal": "🎖️", "target": "🎯",
    "lightning": "⚡", "mega": "📢", "hi": "👋", "premium": "💫", "vip": "🌟"
}
...
def get_country_flag(country_name):
    ...
    for part in name_parts:
        if part in COUNTRY_FLAGS: return COUNTRY_FLAGS[part]
    return "🌍"
    ...
    with data_lock:
        if not os.path.exists(DATA_FILE):
            default_data = {
                ...
                "main_otp_link": "https://t.me/", "watermark": "VIP NUMBER BOT",
        ...
        try:
            with open(DATA_FILE, "r", encoding='utf-8') as f:
                ...
                if not content:
                    return {"users": [], "services_data": {}, "forward_groups": [], "main_otp_link": "https://t.me/", "watermark": "VIP NUMBER BOT", "force_join_enabled": False, "force_join_channels": []}
                ...
                for grp in data.get("forward_groups", []):
                    if "buttons" not in grp:
                        grp["buttons"] = []
                        if grp.get("btn_name") and grp.get("btn_url"): grp["buttons"].append({"name": grp["btn_name"], "url": grp["btn_url"]})
                return data
        except:
            return {"users": [], "services_data": {}, "forward_groups": [], "main_otp_link": "https://t.me/", "watermark": "VIP NUMBER BOT", "force_join_enabled": False, "force_join_channels": []}
...
def get_total_ranges():
    ...
    for srv in data.get("services_data", {}).values():
        for cnt in srv.get("countries", {}).values(): count += len(cnt.get("ranges", {}))
    return count
...
def show_force_join_message(chat_id, message_id=None):
    data = load_data()
    channels = data.get("force_join_channels", [])
    text = f"{emo('warning')} <b>ACCESS DENIED</b> {emo('warning')}\n━━━━━━━━━━━━━━━━━━\n📢 Join our channels to use this bot\n\nClick <b>JOINED</b> after joining"
    markup = InlineKeyboardMarkup()
    for link in channels: markup.add(InlineKeyboardButton(text="📢 Join Channel", url=link))
...
def handle_text(message):
    ...
    if "GET NUMBER" in text:
        if not check_force_join(user_id): show_force_join_message(message.chat.id); return
...
def show_main_menu(chat_id, first_name=None, message_id=None):
    ...
    data = load_data()
    watermark = data.get("watermark", "VIP NUMBER BOT")
    text = (
        f"{emo('crown')} <b>👑 VIP OTP BOT 👑</b> {emo('crown')}\n"
        f"━━━━━━━━━━━━━━━━━━━━━\n"
        f"{emo('hi')} Welcome, <a href='tg://user?id={chat_id}'>{html.escape(first_name)}</a>!\n\n"
        f"✦ Premium OTP Service ✦\n"
        f"✦ 185+ Countries Supported ✦\n\n"
        f"📌 Tap <b>GET NUMBER</b> to start\n"
        f"━━━━━━━━━━━━━━━━━━━━━\n"
...
def show_user_services(chat_id, message_id=None):
    ...
    for srv_id, srv in data.get("services_data", {}).items():
        ...
        if has_ranges: buttons.append(InlineKeyboardButton(text=f"{emo(srv['name'])} {srv['name']}", callback_data=f"usr_s|{srv_id}"))
...
def show_user_countries(chat_id, srv_id, message_id=None):
    ...
    for cnt_id, cnt in srv_data.get("countries", {}).items():
        if len(cnt.get("ranges", {})) > 0: buttons.append(InlineKeyboardButton(text=f"{get_country_flag(cnt['name'])} {cnt['name']}", callback_data=f"usr_c|{srv_id}|{cnt_id}"))
    markup.add(*buttons)
...
def show_user_ranges(chat_id, srv_id, cnt_id, message_id=None):
    ...
    text = f"{emo('number')} <b>SELECT RANGE</b> {emo('number')}\n━━━━━━━━━━━━━━━━━━\n{emo(srv_data['name'])} Service: <code>{html.escape(srv_data['name'])}</code>\n{flag} Country: <code>{html.escape(cnt_data['name'])}</code>\n\nChoose your range"
    safe_send(chat_id, text, markup, message_id)

def show_admin_panel(chat_id, message_id=None):
    data = load_data()
    text = f"{emo('crown')} <b>ADMIN PANEL</b> {emo('crown')}\n━━━━━━━━━━━━━━━━━━\n📊 <b>DATABASE STATS</b>\n━━━━━━━━━━━━━━━━━━\n👤 Users: <code>{len(data.get('users', []))}</code>\n📱 Ranges: <code>{get_total_ranges()}</code>\n🔗 Groups: <code>{len(data.get('forward_groups', []))}</code>\n🌍 Countries: <code>185+</code>\n━━━━━━━━━━━━━━━━━━"
...
def show_admin_countries(chat_id, srv_id, message_id=None):
    ...
    if not srv_data: return
    markup = InlineKeyboardMarkup(row_width=2)
    for cnt_id, cnt in srv_data.get("countries", {}).items(): markup.add(InlineKeyboardButton(text=f"{get_country_flag(cnt['name'])} {cnt['name']}", callback_data=f"adm_c|{srv_id}|{cnt_id}"))
...
def handle_query(call):
    ...
    if call.data in ["main_get_number", "back_to_user_services", "find_number"] or call.data.startswith(("usr_s|", "usr_c|", "chgc|", "chg_r|")):
        if not check_force_join(user_id): show_force_join_message(chat_id, msg_id); return

    if call.data == "check_join":
        if check_force_join(user_id):
            bot.answer_callback_query(call.id, "✅ Welcome to VIP Number BOT!", show_alert=True)
    ...
    if call.data.startswith(("adm_", "add_", "del_", "editgrp_")) or call.data in ["admin_broadcast", "admin_group_settings", "admin_set_watermark", "admin_force_join", "toggle_force_join", "add_fjc", "back_to_admin", "admin_manage_service"]:
        if user_id != ADMIN_ID: return safe_send(chat_id, f"{emo('warning')} <b>Access Denied!</b>", None, msg_id)
    ...
    # Admin callbacks
        ...
        safe_send(chat_id, f"{emo('trash')} <b>OTP Link Removed!</b>", get_group_settings_menu(), msg_id)
        ...
        for g in data.get("forward_groups", []):
            if str(g['chat_id']) == str(grp_id) and 0 <= btn_idx < len(g.get("buttons", [])): g["buttons"].pop(btn_idx); break
        ...
        save_data(data); safe_send(chat_id, f"{emo('trash')} <b>Group Deleted!</b>", get_group_settings_menu(), msg_id)
        ...
        safe_send(chat_id, f"{emo('note')} Send new Watermark:\nCurrent: {data.get('watermark', 'VIP NUMBER BOT')}", markup, msg_id)
...
def process_set_force_join_link(message, msg_id):
    if message.text == '/cancel': safe_send(message.chat.id, f"{emo('mega')} <b>FORCE JOIN</b>", get_force_join_menu(), msg_id); return
    data = load_data(); data.setdefault("force_join_channels", []).append(format_url(message.text.strip())); save_data(data)
    safe_send(message.chat.id, f"{emo('done')} Channel Added!", get_force_join_menu(), msg_id)

def process_add_srv(message, msg_id):
    if message.text == '/cancel': show_admin_services(message.chat.id, msg_id); return
    data = load_data(); srv_id = "s_" + str(uuid.uuid4())[:8]
...
def process_add_cnt(message, srv_id, msg_id):
    if message.text == '/cancel': show_admin_countries(message.chat.id, srv_id, msg_id); return
    data = load_data(); cnt_id = "c_" + str(uuid.uuid4())[:8]
    if srv_id in data.get("services_data", {}): data["services_data"][srv_id]["countries"][cnt_id] = {"name": message.text.strip(), "ranges": {}}; save_data(data)
    show_admin_countries(message.chat.id, srv_id, msg_id)

def process_add_rng(message, srv_id, cnt_id, msg_id):
    if message.text == '/cancel': show_admin_ranges(message.chat.id, srv_id, cnt_id, msg_id); return
    data = load_data(); rng_id = "r_" + str(uuid.uuid4())[:8]
    try: data["services_data"][srv_id]["countries"][cnt_id]["ranges"][rng_id] = message.text.strip(); save_data(data)
    except: pass
    show_admin_ranges(message.chat.id, srv_id, cnt_id, msg_id)

def process_custom_range(message, msg_id):
    if message.text == '/cancel': bot.edit_message_text(f"{emo('cross')} <b>Cancelled.</b>", message.chat.id, msg_id, parse_mode="HTML"); return
...
def process_api_traffic_search(message, msg_id):
    if message.text == '/cancel': bot.edit_message_text(f"{emo('cross')} <b>Cancelled.</b>", message.chat.id, msg_id, parse_mode="HTML"); return
    user_id = message.from_user.id; current_time = time.time()
    if user_id in traffic_cooldowns:
        time_passed = current_time - traffic_cooldowns[user_id]
        if time_passed < 10:
            wait_time = int(10 - time_passed)
            markup = InlineKeyboardMarkup().add(InlineKeyboardButton("❌ Close", callback_data="close_menu"))
            safe_send(message.chat.id, f"{emo('time')} Wait {wait_time}s before checking again.", markup, msg_id); return
    ...
    def check_traffic():
            ...
            if service_query in str(srv.get("name", "")).lower():
                for cnt in srv.get("countries", {}).values():
                    ...
                    for rng in cnt.get("ranges", {}).values():
                        if rng not in ranges: ranges[rng] = {"count": 0, "country": country_name}
        try:
            headers = {'X-API-Key': NEXA_API_KEY, 'Cache-Control': 'no-cache'}
            response = requests.get(f"{BASE_URL}/api/v1/console/logs?limit=500", headers=headers, timeout=15)
            if response.status_code == 200:
                res_data = response.json()
                if res_data.get("success") and res_data.get("data"):
                    for item in res_data["data"]:
                        sms_text = str(item.get("sms", "")).lower(); app_name = str(item.get("app_name", "")).lower()
                        country = str(item.get("country", "Unknown")); num = str(item.get("number", "")); detected = app_name
        ...
        active_ranges = {k: v for k, v in ranges.items() if v["count"] > 0}
        if active_ranges:
            sorted_ranges = sorted(active_ranges.items(), key=lambda x: x[1]["count"], reverse=True)
            res_text = f"{emo('traffic')} <b>Top Ranges for {service_query.title()}:</b>\n━━━━━━━━━━━━━━━━━━\n\n"
            for i, (rng, details) in enumerate(sorted_ranges[:10], 1):
                flag = get_country_flag(details['country'])
                res_text += f"{i}. {flag} {details['country']} → <code>{rng}</code> [{details['count']} OTPs]\n"
            res_text += f"\n━━━━━━━━━━━━━━━━━━\n{emo('note')} Copy range & use in Custom Search!"
        else: res_text = f"{emo('cross')} <b>No active traffic found for {service_query.title()}.</b>\n\n{emo('note')} Try checking again later!"
...
def step1_add_fwd_group(message, msg_id):
    if message.text == '/cancel': safe_send(message.chat.id, f"{emo('link')} <b>Group Settings</b>", get_group_settings_menu(), msg_id); return
    data = load_data(); data.setdefault("forward_groups", []).append({"chat_id": message.text.strip(), "buttons": []}); save_data(data)
    safe_send(message.chat.id, f"{emo('done')} <b>Group Added!</b>", get_group_settings_menu(), msg_id)
...
def step_addgrpbtn_url(message, msg_id):
    if message.text == '/cancel':
        ...
        return
    state = user_states.get(message.chat.id, {}); grp_id = state.get('grp_id'); btn_name = state.get('btn_name'); btn_url = format_url(message.text.strip())
    data = load_data()
    for grp in data.get("forward_groups", []):
        if str(grp['chat_id']) == str(grp_id): grp.setdefault("buttons", []).append({"name": btn_name, "url": btn_url}); break
    save_data(data); safe_send(message.chat.id, f"{emo('done')} <b>Button Added!</b>", None, msg_id)
    show_edit_group_menu(message.chat.id, grp_id, msg_id)

def process_main_otp_link(message, msg_id):
    if message.text == '/cancel': safe_send(message.chat.id, f"{emo('link')} <b>Group Settings</b>", get_group_settings_menu(), msg_id); return
    data = load_data(); data["main_otp_link"] = format_url(message.text.strip()); save_data(data)
    safe_send(message.chat.id, f"{emo('done')} Link Updated!", get_group_settings_menu(), msg_id)

def process_set_watermark(message, msg_id):
    if message.text == '/cancel': show_admin_panel(message.chat.id, msg_id); return
    data = load_data(); data["watermark"] = message.text.strip(); save_data(data)
...
def run_broadcast(chat_id, original_message, msg_id):
    data = load_data(); users = data.get("users", []); success, failed = 0, 0
    for u in users:
...
def fetch_number(chat_id, service_info, api_key, msg_id, is_custom=False):
    ...
    try:
        ...
        if res_data.get("success"):
            number = res_data.get("number"); number_id = res_data.get("number_id")
            api_country = res_data.get("country", service_info['country_name'])
            data = load_data(); watermark = data.get("watermark", "VIP NUMBER BOT")
            flag = get_country_flag(api_country); srv_emoji = emo(service_info['service_name'])
            text = (
                f"{emo('crown')} <b>NUMBER ALLOCATED</b> {emo('crown')}\n"
                f"━━━━━━━━━━━━━━━━━━\n"
                f"{srv_emoji} Service: <b>{html.escape(service_info['service_name'])}</b>\n"
                f"{flag} Country: <b>{html.escape(api_country)}</b>\n"
                f"━━━━━━━━━━━━━━━━━━\n"
                f"📱 <code>{number}</code>\n"
                f"━━━━━━━━━━━━━━━━━━\n"
                f"{emo('fire')} {html.escape(watermark)} {emo('fire')}"
            ...
            markup = InlineKeyboardMarkup(row_width=2)
            if is_custom:
                markup.add(InlineKeyboardButton("🔄 Change", callback_data=f"chgc|{service_info['range']}"), InlineKeyboardButton("📨 OTP Group", url=main_link))
                markup.add(InlineKeyboardButton("❌ Close", callback_data="close_menu"))
            else:
                markup.add(InlineKeyboardButton("🔄 Change", callback_data=f"chg_r|{service_info['srv_id']}|{service_info['cnt_id']}|{service_info['id']}"), InlineKeyboardButton("📨 OTP Group", url=main_link))
...
def poll_otp(chat_id, number_id, phone_number, service_info, api_key, msg_id, is_custom):
    headers = {'X-API-Key': api_key}; timeout = 600; start_time = time.time()
    while time.time() - start_time < timeout:
        ...
        try:
            ...
            if s_data.get("success") and s_data.get("otp"):
                otp_code = s_data.get("otp"); flag = get_country_flag(service_info['country_name'])
                srv_emoji = emo(service_info['service_name']); disp_num = f"+{str(phone_number).replace('+', '')}"

                # ✅ OTP আসার পরেও Change button থাকবে
                main_link = format_url(load_data().get("main_otp_link", "https://t.me/"))
                markup = InlineKeyboardMarkup(row_width=2)
                if is_custom:
                    markup.add(InlineKeyboardButton("🔄 Change", callback_data=f"chgc|{service_info['range']}"), InlineKeyboardButton("📨 OTP Group", url=main_link))
                    markup.add(InlineKeyboardButton("❌ Close", callback_data="close_menu"))
                ...
                inbox_msg = (
                    ...
                    f"🔐 <b>OTP Code:</b>\n<code>{otp_code}</code>\n"
                    f"━━━━━━━━━━━━━━━━━━\n"
                    f"{emo('star')} VIP NUMBER BOT {emo('star')}"
                )
                safe_send(chat_id, inbox_msg)

                # Group forward with flag & service
                masked_num = mask_number(phone_number); srv_short = get_short_service(service_info['service_name'])
                cc = get_iso_code(service_info['country_name'])
                group_msg = (
                    f"╔══════════════════════╗\n"
                    f"║ {srv_emoji} <b>#{srv_short}</b> {flag} <b>#{cc}</b>\n"
                    f"║ 📱 {masked_num}\n"
                    f"╚══════════════════════╝"
                )
                data = load_data()
                for grp in data.get("forward_groups", []):
                    try:
                        ...
                        for btn in grp.get("buttons", []): grp_markup.add(InlineKeyboardButton(text=btn['name'], url=btn['url']))
    ...
    if active_polls.get(str(chat_id), False):
        markup = InlineKeyboardMarkup()
        if is_custom: markup.add(InlineKeyboardButton("🔄 Retry", callback_data=f"chgc|{service_info['range']}"), InlineKeyboardButton("❌ Close", callback_data="close_menu"))
        else: markup.add(InlineKeyboardButton("🔄 Retry", callback_data=f"chg_r|{service_info['srv_id']}|{service_info['cnt_id']}|{service_info['id']}"), InlineKeyboardButton("🔙 Back", callback_data=f"usr_c|{service_info['srv_id']}|{service_info['cnt_id']}"))
        safe_send(chat_id, f"{emo('time')} <b>Timeout!</b> No OTP received.", markup, msg_id)
        active_polls[str(chat_id)] = False

if __name__ == "__main__":
    print("👑 VIP NUMBER BOT - Fully Functional Premium Bot Running... 👑")
